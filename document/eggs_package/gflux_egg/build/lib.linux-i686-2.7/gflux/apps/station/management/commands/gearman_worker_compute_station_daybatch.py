...
class Command(django_gearman_commands.GearmanWorkerBaseCommand):
    @property
    def task_name(self):
        return 'compute_station_daybatch'
    ...
    def do_job(self, job_data):
        try:
            ...
            with open(args['log_file'],'at') as lf:
                lf.write('===new task===\n[%s]start compute station daybatch site:%s\'s data...\n'%(
                    NowTime(),args['site']
                ))
                lf.close()
            ...
            compute_station_daybatch(args['site'],start_date,end_date)

            with open(args['log_file'],'at') as lf:
                lf.write('[%s]end compute station daybatch site:%s\'s data...\n===finish task===\n'%(
                    NowTime(),args['site']
                ))
                lf.close()
        except Exception,e:
...
compute_station_daybatch('FZ_CT',start_date,end_date)
