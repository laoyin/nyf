...
from gflux.apps.station.sql_utils import import_senmei_data
    ...
    def do_job(self, job_data):
        try:
            ...
            with open(args['log_file'],'at') as lf:
                lf.write('===new task===\n[%s]start import site:%s\'s senmei data...\n'%(
                    NowTime(),args['site']
                ))
                lf.close()
            #import
            start_date,end_date=import_senmei_data(args['site'],
                                args['file'],
                                args['location'],
                                args['location_desc'],
                                args['userid'],
            ...
            with open(args['log_file'],'at') as lf:
                lf.write('[%s]end import site:%s\'s senmei data...\n'%(
                    NowTime(),args['site']
                ))
                lf.close()
            ...
            if args['need_update_station_info']:
                with open(args['log_file'],'at') as lf:
                    lf.write('[%s]start update site:%s senmei info...\n'%(
                        NowTime(),args['site']
                    ))
                    lf.close()
                ...
                with open(args['log_file'],'at') as lf:
                    lf.write('[%s]end update site:%s senmei info...\n'%(
                        NowTime(),args['site']
                    ))
                    lf.close()
