...
var pr_fer_pump_type_map={0:gettext("无规律"),1:gettext("加满"),2:gettext("定额")}
...
//big_customer
                            ...
                            $('#big_customer_search').on('click',function(){
                                            ...
                                            $('#slave_card_information').html('<tr><th colspan="4" style="text-align: center">gettext("正在加载数据...")</th></tr>');
                                            $.get('/gcustomer/ajax/big_customer_manage',
                                                            {user_source:1,source_id:10000,name:big_customer_master_cardnum},
                                                            function(data)
                                                            {
                                                                         if(data.ret!='0001'){
                                                                             $("#alert_modal_body").html(data.info);
                                                                            ...
                                                                            $('#infornation_survey').append(String.format('<tr><td>gettext("单位名称")</td><td>{0}</td><td>gettext("创建时间")</td><td>{1}</td></tr>',
                                                                                         data.name,data.create_time))
                                                                             $('#infornation_survey').append(String.format('<tr><td>gettext("已充值量")</td><td>{0}</td><td>gettext("当前余额")</td><td>{1}</td></tr>',
                                                                                         data.prepaid_amount,data.current_balance))
                                                                             $('#infornation_survey').append(String.format('<tr><td>gettext("副卡数量")</td><td>{0}</td><td></td><td></td></tr>',
                                                                                         data.nb_slave_cards))

                                                                            for (var i=0;i<data.slaveObj.length;i++)
                                                                                                $("#slave_card_information").append(String.format('<tr><td>{0}</td><td>{1}</td><td>{2}</td><td>{3}</td><td>{4}</td></tr>',
                                                                                    i+1,data.slaveObj[i].cardnum,data.slaveObj[i].user_name,data.slaveObj[i].curr_balance,gettext('查看')
                                                                                ))
