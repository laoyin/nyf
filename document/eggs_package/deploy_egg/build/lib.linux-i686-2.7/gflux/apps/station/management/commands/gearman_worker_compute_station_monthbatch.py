...
class Command(django_gearman_commands.GearmanWorkerBaseCommand):
    @property
    def task_name(self):
        return 'compute_station_monthbatch'
    ...
    def do_job(self, job_data):
        try:
            ...
            with open(args['log_file'],'at') as lf:
                lf.write('===new task===\n[%s]start compute station monthbatch site:%s\'s data...\n'%(
                    NowTime(),args['site']
                ))
                lf.close()
            ...
            #import
            site = args['site']
            year_date=args['year_date']
            month_date=args['month_date']

            compute_station_monthbatch(site, year_date, month_date)

            with open(args['log_file'],'at') as lf:
                lf.write('[%s]end compute compute station monthbatch site:%s\'s data...\n===finish task===\n'%(
                    NowTime(),args['site']
                ))
                lf.close()
        except Exception,e:
