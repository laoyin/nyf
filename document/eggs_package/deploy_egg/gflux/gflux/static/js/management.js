    ...
    $.post(url_prefix+'ajax/get_channel_machine/',{"site":name},function(data){
        if(data['ret']=='1101'){
            ...
            for(var i=0;i<column.length;i++){
                ...
                name="{0}" class="btn btn-default btn-xs"\
                style="margin-left:10px;margin-top:10px;font-size:16px">{1}\
                         <span class="glyphicon glyphicon-edit" onclick="editModal(this,8);"></span>\
                         <span class="glyphicon glyphicon-minus" onclick="delColumn(this)"></span>\
                 </button>',name,name)
                 $('#column_area').append(tmp)
            }
        }
    },'json')
}
...
function editModal(obj,type){
    ...
    //根据类型弹出对话框,1:新增通道,2:新增油机,3:新增油位,4:编辑通道,5:编辑油机,6:编辑油位，7:新增列道,8:编辑列道
    if(type==1){
        ...
        $('#myModalLabel').text(gettext('新增列道'))
        $('#name').text(gettext('列道名字'))
        ...
        $("#add_item_btn").prop("disabled",false);
        var column=all_channel_machine['column']
        var name=obj.parentNode.name
        $('#myModalLabel').text(gettext('编辑列道'))
        $('#name').text(gettext('列道名字'))
...
function delColumn(obj){
    ...
    //发送请求
    $.post(url_prefix+'ajax/del_column/',{"site":site,"name":name},
        function(data){
            if(data['ret']=='1101'){
                //页面移除该项
                $(obj).parents('button').remove()
