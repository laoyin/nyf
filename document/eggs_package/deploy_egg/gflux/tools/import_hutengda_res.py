...
search_dir_start=os.path.abspath(os.path.dirname(__file__))+'/data/hutengda'
...
def checkDirFiles(dir_path):
    ...
    for p,d,f in os.walk(dir_path):
        ...
        for d_f in f:
            ...
            os.chdir(work_path)
            cmd='python manage.py import_trans_hutengda_text_data --file=%s --location=6 --site=SZ_HUTENGDA >import_log/%s.out'%(res,d_f)
            ret=os.system(cmd)
